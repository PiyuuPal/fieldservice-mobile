...
Codebase for cyberwolftechnician mobile app developed using React Native.
...
The project already has scripts to execute the project calling a specific environment defined into the package.json file. Keep in mind that if you are going to create new `envs` you have to define the script to build the project properly.
...
The idea of this section is to explain how the project composition is the best and easiest to use when you try to use well-formed, architectures, especially using redux flow.

The project follows a simple and convenient exporting pattern. The folder index exposes the resources, allowing to import all from the same path.
...
To keep the networking layer simple, the project uses a single Axios instance in the `httpClient.js`. It uses interceptors to define common side effects for the responses.
